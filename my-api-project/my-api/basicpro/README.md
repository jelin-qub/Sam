...
```bash
basicpro$ sam build --use-container
```
...
```bash
basicpro$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
basicpro$ sam local start-api
basicpro$ curl http://localhost:3000/
```
...
```bash
basicpro$ sam logs -n HelloWorldFunction --stack-name "basicpro" --tail
```
...
```bash
basicpro$ pip install -r tests/requirements.txt --user
# unit test
basicpro$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
basicpro$ AWS_SAM_STACK_NAME="basicpro" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "basicpro"
```
